...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, AgentProfilePresenterComponent, RouterLink, DragDropModule],
...
export class AdminAgentesListComponent {
  ...
  remove(row: AdminAgent, dlg?: HTMLDialogElement) {
    ...
    // ===== Drag & Drop (ordenar agentes) =====
  drop(event: CdkDragDrop<AdminAgent[]>) {
  ...
  // enviar al backend
  this.api.reorder(payload).subscribe({
    error: () => {
      this.errorMsg.set('No se pudo guardar el orden');
    },
  });
}
